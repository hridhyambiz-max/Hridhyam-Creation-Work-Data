...
async function addEmployee(e) { e.preventDefault(); const data=Object.fromEntries(new FormData(e.target).entries()); const departments=formDepartments(e.target); if(!departments.length) return toast("Select at least one department","error"); delete data.departments; if(state.employees.some(x=>x.empId===data.empId||x.email===data.empId)) return toast("Employee ID already exists","error"); await addDoc(collection(db,"employees"),{...data,department:departments[0],departments,email:data.empId,status:"Active",createdAt:serverTimestamp()}); e.target.reset(); toast("Employee added"); }
