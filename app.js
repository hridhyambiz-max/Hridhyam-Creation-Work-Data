...
const empDepartments = (e) => Array.isArray(e?.departments) && e.departments.length ? e.departments : (e?.department ? [e.department] : []);
const deptListText = (e) => empDepartments(e).map((d) => deptName(d)).join(", ") || "-";
const hasDept = (e, dept) => empDepartments(e).some((x) => x === dept || deptName(x) === dept || x === state.departments.find((d) => d.name === dept)?.id);
...
function ensureStyles() {
  ...
  *{box-sizing:border-box} body{margin:0;font-family:Inter,system-ui,Segoe UI,Arial,sans-serif;background:linear-gradient(135deg,#e3f7ff,#f4f5ff);color:var(--text)} button,input,select,textarea{font:inherit} button{cursor:pointer;border:0}.hidden{display:none!important}
  ...
  .check-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px}.check-item{display:flex;align-items:center;gap:8px;background:#fbfdff;border:1px solid #e6eef7;border-radius:12px;padding:10px;font-weight:800;color:#526177}.check-item input{width:auto!important;margin:0}.dept-tags{display:flex;gap:6px;flex-wrap:wrap}
...
function departmentEmployeeList() {
  return state.departments.map((d) => {
    const emps = state.employees.filter((e) => hasDept(e, d.name) || hasDept(e, d.id));
...
function employeesView() {
  ...
  return `<div class="card"><div class="section-title"><h3>Add Employee</h3></div><form id="empForm" class="form-grid"><input name="name" placeholder="Employee name" required><input name="empId" placeholder="Employee ID" required><input name="email" type="email" placeholder="Email ID"><input name="password" placeholder="Password" required><input name="phone" placeholder="Phone"><input name="emergencyContact" placeholder="Emergency contact"><div class="wide"><label class="muted" style="font-weight:800">Departments (multiple select)</label><div class="check-grid">${deptCheckboxes()}</div></div><input name="role" placeholder="Role"><input name="salary" type="number" placeholder="Monthly salary"><input name="paidLeave" type="number" placeholder="Paid leave" value="${esc(state.settings.defaultPaidLeave || 2)}"><input name="photoUrl" placeholder="Profile photo URL"><button class="btn primary">Add Employee</button></form></div><div class="card" style="margin-top:18px"><h3>Employees</h3><div class="table-wrap"><table class="table"><tr><th>Employee</th><th>ID</th><th>Email</th><th>Department</th><th>Salary</th><th>Leave</th><th>Points</th><th>Status</th><th>Action</th></tr>${state.employees.map((e) => { const lb = leaveBalance(e); return `<tr><td><div class="profile-row">${profileAvatar(e)}<b>${esc(e.name)}</b></div><div class="muted" style="font-size:12px">${esc(e.phone || "")}${e.emergencyContact ? ` • Emergency: ${esc(e.emergencyContact)}` : ""}</div></td><td>${esc(e.empId)}</td><td>${esc(e.email || "-")}</td><td><div class="dept-tags">${empDepartments(e).map(d=>`<span class="badge blue">${esc(deptName(d))}</span>`).join("") || "-"}</div></td><td>${money(e.salary)}</td><td>${lb.remaining} left / ${lb.used} used / ${lb.extra} extra</td><td>${monthlyTaskPoints(e.id)}</td><td><span class="badge green">${esc(e.status || "Active")}</span></td><td><div class="row-actions"><button class="btn small" data-edit-emp="${e.id}">Edit</button><button class="btn small danger" data-del-emp="${e.id}">Delete</button></div></td></tr>`; }).join("")}</table></div></div>`;
}
...
function deptCheckboxes(selected = []) {
  ...
  return state.departments.map((d) => {
    const checked = arr.includes(d.name) || arr.includes(d.id) ? "checked" : "";
    return `<label class="check-item"><input type="checkbox" name="departments" value="${esc(d.name)}" ${checked}> ${esc(d.name)}</label>`;
  }).join("") || `<p class="muted">No departments added yet.</p>`;
}
function formDepartments(form) { return Array.from(form.querySelectorAll('input[name="departments"]:checked')).map((x) => x.value); }
...
async function addEmployee(e) { e.preventDefault(); const data=Object.fromEntries(new FormData(e.target).entries()); const departments=formDepartments(e.target); if(!departments.length) return toast("Select at least one department","error"); delete data.departments; const cleanEmail=(data.email||"").trim(); if(state.employees.some(x=>x.empId===data.empId)) return toast("Employee ID already exists","error"); if(cleanEmail && state.employees.some(x=>(x.email||"").toLowerCase()===cleanEmail.toLowerCase())) return toast("Email ID already exists","error"); await addDoc(collection(db,"employees"),{...data,email:cleanEmail,department:departments[0],departments,status:"Active",createdAt:serverTimestamp()}); e.target.reset(); toast("Employee added"); }
...
async function editEmployee(id){ const e=empById(id); if(!e) return; await modal({title:"Edit Employee", submitText:"Save Changes", body:`<div class="form-grid"><div class="field"><label>Name</label><input name="name" value="${esc(e.name)}" required></div><div class="field"><label>Employee ID</label><input name="empId" value="${esc(e.empId)}" required></div><div class="field"><label>Email ID</label><input name="email" type="email" value="${esc(e.email||"")}" placeholder="employee@email.com"></div><div class="field"><label>Password</label><input name="password" value="${esc(e.password)}" required></div><div class="field"><label>Phone</label><input name="phone" value="${esc(e.phone||"")}"></div><div class="field"><label>Emergency Contact</label><input name="emergencyContact" value="${esc(e.emergencyContact||"")}"></div><div class="field wide"><label>Departments (multiple select)</label><div class="check-grid">${deptCheckboxes(empDepartments(e))}</div></div><div class="field"><label>Role</label><input name="role" value="${esc(e.role||"")}"></div><div class="field"><label>Salary</label><input name="salary" type="number" value="${esc(e.salary||0)}"></div><div class="field"><label>Paid Leave</label><input name="paidLeave" type="number" value="${esc(e.paidLeave||state.settings.defaultPaidLeave||2)}"></div><div class="field wide"><label>Profile Photo URL</label><input name="photoUrl" value="${esc(e.photoUrl||"")}"></div></div>`, onSubmit: async(data)=>{const departments=Array.from(document.querySelectorAll('.modal-form input[name="departments"]:checked')).map(x=>x.value); if(!departments.length) throw new Error("Select at least one department"); delete data.departments; const cleanEmail=(data.email||"").trim(); if(state.employees.some(x=>x.id!==id && x.empId===data.empId)) throw new Error("Employee ID already exists"); if(cleanEmail && state.employees.some(x=>x.id!==id && (x.email||"").toLowerCase()===cleanEmail.toLowerCase())) throw new Error("Email ID already exists"); await updateDoc(doc(db,"employees",id),{...data,email:cleanEmail,department:departments[0],departments}); toast("Employee updated");}}); }
